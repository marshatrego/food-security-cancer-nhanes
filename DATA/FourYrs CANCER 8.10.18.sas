...
data work.medq_fsq_4yr_all;
	...
	if mcq230a=32 and mcq230b=. or mcq230a=33 and mcq230b=. then NMSkin='Yes';
	else NMSkin='*No';
...
proc surveylogistic data=work.medq_fsq_4yr_all;
	...
	weight wtint4yr;
	class  
	age (param=ref ref=first) time_diag2 (param=ref ref=first) 
	breastcanc (param=ref ref=first)
											digestcanc (param=ref ref=first)
											frepcanc (param=ref ref=first)
											mrepcanc (param=ref ref=first)
											melanoma (param=ref ref=first)
											nmskin (param=ref ref=first)
											othercanc (param=ref ref=first)
											multcanc (param=ref ref=first)
		riagendr (param=ref ref=first) ridreth1 (param=ref ref=first) partner (param=ref ref=first) children (param=ref ref=first)
		 insurance  (param=ref ref=first) education (param=ref ref=first) pir (param=ref ref=first);
	
	format ridreth1 raceth. riagendr gender. mcq220 cancer.;
	model fi_bin (event='Food Insecure')= breastcanc
											digestcanc
											frepcanc
											mrepcanc
											melanoma
